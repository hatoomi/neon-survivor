# USER.md — Mohammed Alhatem Profile v2.0
# Upgraded: 2026-03-27

## Identity
Name: Mohammed Alhatem
Call him: Mohammed
Location: Dubai, UAE
Timezone: Asia/Dubai (GST / UTC+4)
Telegram ID: 5427517880
WhatsApp: +971501885474

## Trading Profile
Instrument: BTC/USDT.P — perpetual futures on Binance
Style: Institutional swing trading + intraday
Sessions: London Open + London/NY overlap (primary)
 Asian session (observation only — no entries)
Dead Zone: 00:00-02:00 Dubai — no entries ever
Methodology: Smart Money Concepts (SMC) + ICT + Macro Liquidity

## Risk Rules (Non-Negotiable)
Max risk per trade: 2% of account (standard)
Max risk in uncertainty: 0.5% of account
Reversal trade risk: 1% of account
Max open positions: 1 at a time
Daily loss cap: Stop after 3 consecutive losses
Blackout windows: FOMC, CPI, NFP, PCE — ±30 min no entries
EM-5 / EM-6 phases: NO TRADE — fully suspended
EM-3 phase: No new longs only

## Trading Vocabulary
BOS — Break of Structure
CHoCH — Change of Character
OB — Order Block
FVG — Fair Value Gap
POI — Point of Interest
CVD — Cumulative Volume Delta
GEX — Gamma Exposure
MVRV — Market Value to Realized Value
NUPL — Net Unrealized Profit/Loss
SOPR — Spent Output Profit Ratio
HVN — High Volume Node
LVN — Low Volume Node
POC — Point of Control
VAH — Value Area High
VAL — Value Area Low
ELG — EM Composite Liquidity Gauge
EM Phase — Expansion/Contraction macro phase (EM1-EM6)

## Macro Framework
Uses Michael Howell / CrossBorder Capital liquidity framework
ELG formula: 0.40 x (-Z(SOFR-Fed Funds)) + 0.30 x (-Z(delta TGA)) + 0.30 x (-Z(delta RRP))
Tracks: Fed balance sheet, TGA, RRP, SOFR, global M2
EM phases EM1-EM6 determine directional bias

## Data Sources
CoinGlass — primary real-time data (screenshots as input)
altFINS MCP — OHLCV candles, altcoin scanning
LunarCrush MCP — AltRank, sentiment
Deribit / Laevitas — options, derivatives
CryptoQuant — on-chain
FRED — macro data
Coinalyze — funding, OI

## Tech Stack
VPS: Hostinger (IP: 187.77.139.185)
Agent: OpenClaw (Node.js) — running on VPS
GitHub: hatoomi/neon-survivor (main branch)
TradingView — charts and Pine Script indicators
AI models: Claude Sonnet (OpenClaw agent)

## Communication Style
Direct. Command-style. Often all-caps.
No padding. No preambles. Lead with the answer.
Prefers concise and actionable over lengthy explanations.
Expects institutional-grade outputs.
Does not need things explained twice.
Risk-first mindset — always flag risk before opportunity.

## What Mohammed Expects
- Sharp analysis with no fluff
- Signal over noise always
- Risk flagged clearly and early
- Probability-based scenarios not predictions
- Data before opinion
- Instant answers when possible
- Proactive alerts on important developments
- System always backed up and memory always current

## Active Projects (as of 2026-03-27)
1. ONYX ELITE — crypto intelligence terminal
 - Activated 2026-03-27
 - Next: data pipelines (stablecoins, ETF flows, ELG calculator)

2. BTC DATA AGENT — metrics collection framework
 - Next: RSI, Deribit options, Pi Cycle, Taker Buy/Sell

3. ALTFINS SYSTEM — altcoin scanning engine
 - Next: live feeds, 8-prompt scanning engine

## END OF USER PROFILE
